# Hi everyone, I'm Ahsanul Jasir Rafif Ilhami, S.T. 👋
**Web Developer | Data Analyst | Informatics Engineer**

A graduate in Informatics Engineering from **Institut Teknologi PLN (ITPLN)** (Class of 2025). I specialize in building web applications and performing data-driven analysis using machine learning techniques.

---

### 🚀 Career Focus

* **Web Development:** Experienced in developing inventory management systems using **CodeIgniter 4**, PHP, and MySQL during my internship at **PT PLN UID Banten**.
* **Data Analytics:** Specialized in **Hierarchical Clustering** algorithms (Complete & Single Linkage) to analyze and group power consumption data.
* **Background:** Proficient in both Front-end and Back-end development, certified by **Dicoding Indonesia**.

---

### 🛠️ Languages, Skills & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=php,py,html,css,js,mysql,postgres,bash,linux,ubuntu,vscode,jupyter,windows,git,github,docker" alt="My Tech Stack" />
</p>
### 📊 GitHub Insights
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=aji-techno&show_icons=true&theme=radical&hide_border=true" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=aji-techno&layout=compact&theme=radical&hide_border=true" height="150" />
</p>

---

### 🏆 Achievements & Certifications
* **MTCNA:** MikroTik Certified Network Associate.
* **Studi Independen:** Front-end & Back-end Development Cohort at Dicoding Indonesia (2024).
* **Leadership:** Former Head of Student Resource Development (PSDM) at HMTI ITPLN.

---

### 🕹️ Contribution Games
**Pacman Activity**
![Pacman](https://raw.githubusercontent.com/aji-techno/aji-techno/output/github-snake.svg)

**Snake Activity**
![Snake](https://raw.githubusercontent.com/aji-techno/aji-techno/output/github-snake-dark.svg)

